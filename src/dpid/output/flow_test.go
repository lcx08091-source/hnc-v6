...
func TestObserveCreatedFlag(t *testing.T) {
	...
	if _, created := ft.observe("T|1.1.1.1:443", now, 100); !created {
		t.Fatal("first observe must report created=true")
	}
	if _, created := ft.observe("T|1.1.1.1:443", now+1, 100); created {
		t.Fatal("second observe on same flow must report created=false")
	}
	if _, created := ft.observe("U|1.1.1.1:443", now+2, 100); !created {

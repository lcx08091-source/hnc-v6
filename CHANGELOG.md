...
- **定时开关热点 / 只在充电时开热点**：`rules.json` 里一直有 `hotspot_time_*` / `hotspot_charging_only` 字段但从没被读取，现在由新脚本 `bin/hotspot_schedule.sh` 落地(watchdog 每轮检查，开机自启先询问)。只在「应该开 / 应该关」发生切换时动作，时段外手动开的热点不会被反复关掉；支持跨午夜时段。设置 → 热点 → 定时与充电。
- **离线规则自动清理天数**可在界面修改(设置 → 维护，0 = 永不清理)。
- **设备卡显示本月已用流量**，开了月度配额时附占比并按预警/超额变色；新接口 `GET /api/usage_month`。
- 新动作 `hotspot_schedule_set`、`stale_ttl_set`；`/api/live?devices=1` 一次返回汇总与设备列表。

### Fixed

- **时区**：Android 上 Go 的 `time.Local` 恒为 UTC —— 免打扰时段错开 8 小时(白天静音、半夜推送)，今日统计 / 本月流量 / 在线时长 / 自然月的零点偏 8 小时。dpid、watchdog、httpd 统一改用真实系统时区。
- **月度配额每月 8 号后少算**：历史文件只保留 7 天，配额只统计到最近 7 天 —— 保留期提到 32 天。
- 限速核心路径：
  - 某些设备没有 mark_id 时会读到下一台设备的 mark_id，两台设备限速互相串。
  - mark_id=1 的设备(MAC 末字节 00/63/c6)撞上 HTB 父类，限速/延迟永远失败，清限速还会让全局整形失效。
  - 清一台设备的限速会误删下一台设备的过滤器，导致其上行不再限速。
  - 应用限速的清理范围误含默认类，每 30 秒删一次默认类，全局整形与默认队列失效。
  - IPv6 过滤器在热点重开 / 重启 / 换接口后不重建，走 IPv6 的设备绕过限速；残留过滤器会把旧设备的流量归到别人名下。
  - 清理离线设备规则时自己把自己锁死，陈旧的打标规则永远删不掉。
  - 热点换接口后最长 5 分钟规则打在已消失的旧接口上；ColorOS 关开热点重建同名接口后规则丢失且不恢复。
  - 关机时恰好持锁，开机后锁永远拿不到，恢复和限速全部失败。
  - 换接口后白名单的拦截规则还挂在旧接口上，新接口不再拦截。
  - 每次设上行限速都重装一遍镜像规则(检测字符串对不上)，期间上行不整形。
  - 每次开热点 watchdog 主循环白等 15 秒以上。
  - HTB 挂在多队列下时健康检查恒失败，每轮把设备规则整棵清空。
  - iptables 重建后恢复失败时黑名单设备会恢复上网。
- DPI 守护进程与 C 代码：
  - watchdog 日志满 1MB 时自死锁，之后规则恢复、告警全部静默停止。
  - 热点重开后抓包可能永久失明(重绑时旧句柄过早关闭)。
  - IPv6 / 双栈连接查不到所属 App，本机应用归因大面积落空；某接口抓包失败后永远不重启。
  - 告警已读 / 已知设备账本非原子写、多进程共用临时文件，可能写坏导致告警全部变回未读、所有设备重新报陌生。
  - 状态文件多写者互踩写出坏 JSON；指纹表无上限内存增长；多处外部命令无超时可能永久卡住；eBPF 采样传栈地址给内核。
  - launcher 关停时的信号竞态与无限等待；supervisor 直接 SIGKILL 导致误判崩溃循环。
- WebUI 交互(触摸自动化测试复现):
  - 在限速/延迟框里输入时, 只要有别的设备变化(如新设备连入), 列表重绘把输入清空、键盘收起。
  - 长按封锁: 按住后手指滑走照样会封锁; 按得稍久一点松手, 会点中就地出现的「解除封锁」→ 刚封锁就被解封。
  - **点底栏标签偶尔跳错页**(在设备点应用跳到分析、在设置点应用跳到设备): 松手时用透镜自身的弹簧速度算"甩"的惯性, 轻点时透镜正以 1000~2200px/s 冲向目标, 被当成甩多滑一格。现在轻点直接去手指下的标签; 拖动只在手指真的快甩(>700px/s)时加惯性, 最多多滑半格。
  - 底栏透镜切到最左/最右标签时回弹冲出底栏约 30px。
  - 柱状图柱子仅约 9px 宽很难点中, 改为点图表任意位置选最近的柱子。
  - 新增: 弹层可以下拉关闭。
- `ndpi-lab.html` 输出永远显示 0(回调签名错误)。
- 白名单同步在首次初始化时能拿到热点接口，不再退化成不限接口的拦截。

### Changed

- 省电与性能：本机界面每轮轮询合并为一次请求；远程浏览器改用实时推送，空闲时轮询放缓到 15~30 秒、切后台断开；远程访问的 cookie 校验结果缓存 5 分钟(不再每个请求一次 bcrypt)；授权设备列表只在有变化时写盘；`/api/iface_info` 不再每次拉起 shell 脚本。

### Internals

- 测试框架：独立风格的测试此前成败都不计数(失败也显示 ALL PASS)，现在按退出码记账；由此暴露的失败已修正，删除 8 个验证已于 v5.10.0 退役的 v5.2 统计灰度门禁的测试。新增定时热点、限速核心路径(16 条)、时区、配额保留期等回归测试。全量 ALL PASS 250/251。
- 需要 CI 重编：hnc_httpd、hnc_dpid、hnc_watchdog、hnc_dpid_supervisor、hnc_launcher。

---

## [5.11.0] - 2026-09-23

**新版 WebUI 正式上线 + 全量排查修复**。新界面替换原主界面(旧界面保留为「经典界面」),前后端功能逐项对齐；同时修了后端、JSON 写入层、白名单等一批问题。

### Added

- **新版 WebUI**(`webroot/index.html`)：沿用原版统计页的设计语言，二级内容一律就地展开；平板为「列表 + 右侧常驻详情」分栏；底部标签栏按住变液态玻璃透镜，可左右拖动、松手弹簧吸附，边缘有色散折射(Hyalite，VII-Cae，MIT)；页面切换、分段控件、开关等统一为弹簧动画，可在设置里关闭。
  - 同一份页面同时支持本机 KernelSU WebView(桥接 `127.0.0.1:8444`)与远程浏览器(同源 + 配对 cookie)；远程浏览器现在也拿到完整功能，不再是精简版。
  - 覆盖旧界面全部功能：设备限速/延迟/丢包/低延迟/白名单/应用限速/7 天曲线/重命名/长按封锁、批量、模板、全局操作、告警、本机应用归因与候选审核、数据导出、流量统计、DPI 分析与规则库、设置页全部分组。
  - 旧界面保留为 `classic.html`(设置 → 关于 → 经典界面)。
- 后端新增接口，取代旧前端直接在手机上跑 shell 改状态(绕过写锁与审计、远程不可用)：
  - 动作 `device_whitelist_set`、`qos_set`、`template_set`、`template_del`、`cache_clear`、`debug_bundle`、`dpi_rules_reset`、`dpi_rules_update`。
  - 只读 `GET /api/rules_export`(热点密码脱敏)、`/api/run_status`、`/api/proc_health`；`GET/POST /api/dpi_rules`(规则库导出 / 导入，≤512KB)。
  - `/api/config` 补 `hotspot_iface`、`tc_qos_mode`、`tc_qos_scale`、`qos_fallback`；`/api/devices` 补 `whitelist`，离线设备行补 `sqm_enabled` / `whitelist`。
  - `/api/exports` 同时列出诊断包(`.tar.gz`)，远程可下载。
- `docs/API.md`：hnc_httpd 完整接口参考。
- `bin/whitelist_sync.sh`：白名单模式落到 iptables 的唯一入口(httpd 开关 / 单设备白名单 / watchdog 重建 iptables 后都会调用)。

### Fixed

- **白名单模式从来不生效**：开关只改了 rules.json，没有任何地方调用 iptables 放行/拦截；另外链尾裸 `DROP` 挂在 FORWARD 上会连外网回来的应答包一起丢，白名单设备自己也会断网——现在先放行已建立连接，`DROP` 只拦热点接口进来的新连接。
- **设备「今日在线时长」永远是空的**：watchdog 采样时把变量当命令执行，写出非法 JSON；读侧也增加了坏行兜底，已落盘的历史能恢复。
- **统计页「今日 · 应用归因」恒为 0**、近 7/30 天首日只统计了一半；DPI 历史按 UTC 日期找文件，东八区凌晨数据读不到。
- 脚本超时后子进程占着输出管道不退，httpd 写锁永久卡死，之后所有操作都没反应。
- 远程页面的实时推送(SSE)一直 500，只能退回轮询。
- 配对 PIN 只按来源 IP 限流，换地址即可无限重试——增加全局上限。
- 证书与私钥替换中途崩溃会导致远程访问永久起不来——启动时校验配对，不匹配就重新生成。
- 速率参数超大数字可溢出绕过 10 Gbit 上限；数据导出可无限次、并发生成(现与写操作共用限流并串行化)。
- JSON 写入层：模板名含反斜杠会被改写且删不掉；纯数字的热点密码/SSID 被存成数字(开机自启读不到、前导 0 丢失)；并发写可能丢条目，收到 TERM 后会在无锁状态继续写；备份文件无限增长；`cfg_set` 在空对象上写出非法 JSON；`json_doctor` 恢复不是原子替换；`json_set_batch` 的临时参数文件可能错位或被符号链接截断。
- 旧界面遗留问题在新界面里全部修正：保存热点配置会把「开机自启」关掉；应用页所有提示不显示；模板里的延迟/抖动/丢包从不生效；设备卡的应用限速列表基本是空的；配对快捷链接不会预填 PIN；桥接模式把 HTTP 401/500 当成功静默吞掉；本地密钥首次读取失败后整页永远 401；启动失败重试成功后配置/能力/告警都停在默认值；告警面板「拉黑」绕过网关保护。
- 清理离线设备、白名单同步在 rules.json 为多行格式时匹配不到设备块(加固)。

### Removed

- 4 个只针对 v5.2-rc1.6~rc1.10 旧传输状态机、并写死版本号的过期测试(`test_webui_bridge_hint_rc1_10` 等)；其余旧界面测试改为检查 `classic.html`，并新增 `test_webui_v6.sh`(校验前端用到的每个动作和接口在后端都存在)。

---

## [5.10.6] - 2026-09-23

**WebUI 新版界面预览 v4**：修三处视觉 bug，底栏透镜加色散折射。不改动线上 WebUI。

### Fixed

- 选中的胶囊按钮顶部有一道白色高光线（看起来像留白）——选中态改用独立的投影。
- 主题切换的圆形扩散：旧画面被 View Transition 默认的淡出冲掉，扩散圈外一片空白、像卡住——关掉默认交叉淡化并加 1.2 秒兜底。
- 底栏透镜按下时从蓝色瞬间变白——蓝色改为独立一层，0.4 秒淡出过渡。

### Added

- 底栏透镜边缘的色散折射：Chromium / Android WebView 上由 Hyalite（VII-Cae，MIT）做真折射 + 三通道色散，另叠 CSS 红青色散描边；透镜下方原图标用遮罩挖掉，避免重影。

---

## [5.10.5] - 2026-09-23

**WebUI 新版界面预览 v3**：回到原版的视觉语言，补齐缺失的功能，底部标签栏改成可拖动的液态玻璃透镜。不改动线上 WebUI，刷机包内容不变。

### Changed

- `docs/webui-v2/index.html` 重做:
  - 全局沿用原版统计页的卡片、计数格、渐变柱状图和分段控件样式；二级内容全部改为就地展开的卡片，不再跳页。平板上设备页为 列表 + 右侧常驻详情。
  - 补齐原版功能：设备过滤、全局操作、批量/模板、延迟抖动丢包、白名单；应用页追踪概览/接口切换/未匹配 SNI/导出/追踪开关；分析页流量统计 + DPI 分析；设置页原版全部分组。
  - 底部标签栏：按住变成液态玻璃透镜（放大下方图标），可左右拖动，松手按弹簧物理吸附到最近的标签并切页，带惯性、挤压拉伸和边缘橡皮筋。
  - 动画统一为物理弹簧曲线（采样成 CSS `linear()`），页面切换方向感知，各控件加入回弹/果冻效果；支持关闭动画。

---

## [5.10.4] - 2026-09-23

**WebUI 新版界面预览改为 iOS 分栏风格**。不改动线上 WebUI，刷机包内容不变。

### Changed

- 设计预览由 `docs/webui-glass/`(液态玻璃)换成 `docs/webui-v2/`:浅色 / 深色 iOS 风格，平板左右分栏(列表 + 详情)，手机推入式详情页 + 边缘右滑返回；顶部导航栏大标题随滚动收起并变为毛玻璃模糊；列表浮现、弹簧指示块、按压缩放、图表生长等动画。

### Removed

- `docs/webui-glass/` 及其引入的 Hyalite 引擎(不再使用)。
...
- **post-fs-data.sh 编辑事故**(v5.9.92 引入):LSM 部署块删除时把文件头约 96 行整体复制了一份(umask/规则初始化/资源拷贝全部跑两遍)、留下裸 `system/bin/sh` 残行,而真正的 LSM 部署块反而没删成。本版从 git 基线恢复文件后在干净副本上重做删除(已验证:umask 仅 1 处、裸行 0 处、LSM 引用 0 处、sh -n 通过)。
- **告警配置的两个开关是死按钮**(v5.10.0 引入):toggle 元素漏写 `data-action` 属性——点击只翻视觉状态,不发任何后端调用、不落盘;且两个保存按钮硬编码 `enabled:'true'`,UI 上不存在"关闭"路径。已补属性。
- **规则库在线更新 --install 必然失败**(三重断链,全部修复):①CI 产物是裸 `dpi_rules.json` 而脚本找 `dpi-rules.zip` 附件——CI 改为产出真正的 zip;②脚本的 `sed "s/.*://"` 贪婪匹配会剥掉 `https:` 的冒号,URL 变成 `//github.com/...` curl 拒绝——改为按 key 边界剥离并实测验证;③sha256 附件里是 JSON 的哈希而校验对象是 zip——改为对 zip 本身计算。另补上脚本头注释声称却从未实现的版本比对(相同版本跳过重装)。
- **「今日在线 X 小时」永不显示**:前端在脚本求值期(设备卡渲染之前)一次性拉取并填充,之后设备卡每次重渲都会抹掉已填文本且无重试。改为具名函数,renderDeviceList 每次渲染后调用。

### Fixed(Important/Minor)

- 「检查规则库更新」按钮在确认弹窗取消后永久禁用(confirmModal 无 onCancel 钩子)——检查阶段不再禁用,仅安装阶段禁用。
- 在线时长采样排除拉黑设备(被拉黑的不算"在线")。
- 统计来源切换的 toast 文案三态各自正确(之前 dpi 也显示"旧统计")。
- 若干注释纠错(灰度旗标退役说明等)。

### 已知待办(不变)

月度配额/anomaly 的配置 UI 本版已落地,但 clsact BPF 仍未真机验证;post-fs-data.sh 的恢复已通过 sh -n 与白名单完整性复核。

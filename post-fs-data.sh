...
# v5.9.92 (v5.10.1 修正): BPF LSM Limit Map Guard 部署块【删除】。
# 三方不一致的收口: hotspotd 链接的是恒 DISABLED 的 stub(v5.8.7: 真 loader
# 需 libelf/libz, 无可用 Android 版); 打包又用 -x "bpf/*" 把 .o 排除在 zip 外
# → 本块恒为 no-op, 只会误导读者。源码(lsm/*.c)保留作参考, 待真 loader 可编
# 时恢复此块与 build.sh 编译块。(v5.9.92 的删除曾把本文件改坏, v5.10.1 从
# git 基线重做。)
...
# 三级轮转:.log.2 → 删除,.log.1 → .log.2,.log → .log.1,新建空文件
# 保留约 3/4 的历史(相比之前单级轮转只保留 1/2)
LOG_MAX_BYTES=$((10 * 1024 * 1024))
for logf in "$HNC_DIR"/logs/*.log; do
    [ -f "$logf" ] || continue
    size=$(wc -c < "$logf" 2>/dev/null | tr -d ' ')
    if [ -n "$size" ] && [ "$size" -gt "$LOG_MAX_BYTES" ]; then
        # Rotate: .log.2 → deleted, .log.1 → .log.2, .log → .log.1
        [ -f "${logf}.2" ] && rm -f "${logf}.2"
        [ -f "${logf}.1" ] && mv "${logf}.1" "${logf}.2"
        [ -f "$logf" ] && mv "$logf" "${logf}.1"
        touch "$logf"
        echo "[HNC] logrotate: $logf rotated (was ${size} bytes, 3-level)" >> "$HNC_DIR/logs/boot.log"
    fi
done
